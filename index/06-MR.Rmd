...
## Introduction
<!-- think about re-shaping introduction to show what found in chapter 4 and how the general limitations of observational analyses will likely apply – this will lead into the MR info and then the methods -->
...
Here, a parallel MR analysis using multiple instruments identified in Chapter \@ref(instrumentation) of BMI, WHR, and BF with 123 metabolites from Kettunen et al. (2016)[@Kettunen2016] (N = 24,925) and 230 metabolites from INTERVAL (unpublished; N = 40,849) is performed. Follow-up meta-analyses of p-values for 110 metabolites in up to 65,774 individuals of European ancestries, and comparison with observational analyses in adults is used to identify persistent effects across studies and methods. \par

<!--
Wurtz et al. (2014) investigated the association between body mass index (BMI) with 82 metabolic measures (including systolic and diastolic blood pressure) in 12,664 individuals. 

Bull et al. (2014) investigated the association between BMI and waist hip ratio (WHR) with 123 metabolic measures in 24,925 individuals.

Bull et al. (2020) investigated the association between BMI and WHR with 249 metabolic measures in 109,532 individuals.
...
The NMR analysis was performed with the comprehensive quantitative serum/plasma platform described by Soininen et al. (2009)[@Soininen2009]. Some samples or analytes were flagged as being potentially unreliable which was denoted by a set of ‘flag’ variables. A number of individuals had flagged values for acetate, pyruvate, glucose, and lactate. Flagged values were set to missing for individuals where they had been flagged. Metabolites with 0 values or values > 10 SD from the mean after log transformation were set to missing. Individuals were excluded if they had > 30% analyte missingness, and one NMR PCA outlier. All metabolites were log transformed and then adjusted for age, sex, BMI, recruitment centre, time between blood draw and sample processing, and the first 10 PCs of genetic ancestry. Residuals were then inverse normal rank transformed prior to GWAS. \par

SNPs were imputed using a combined 1000 Genomes + UK10K panel, which captured 87,696,910 variants. A linear mixed model was fit using BOLT-LMM. Variants not in HWE (p < XXX), associations with XXX were excluded. A genome wide significance threshold was not set. A total of XXX principal components explained over 95% of variation in the metabolite data. \par
...
An inverse variance weighted (IVW), multiplicative random effects (IVW-MRE) model was used to investigate the association of each exposure on each metabolite. The model assumes that the strength of the association of the genetic instruments with the exposure is not correlated with the magnitude of the pleiotropic effects and that the pleiotropic effects have an average value of zero[@Bowden2017]. A multiple testing threshold of *p-value* < `r round(0.05/22,4)` was applied to the Kettunen et al. (2016) metabolomics data. This is based on the number of principal components (22) in the Kettunen et al. (2016) meta-analysis that explained 95% of the variation in metabolite data. A multiple testing threshold of *p-value* < `r round(0.05/9999,4)` was applied to the INTERVAL metabolomics data. This is based on the number of principal components (9999) in the INTERVAL GWAS that explained 95% of the variation in metabolite data. \par
...
bf <- subset(main, exposure == "BF_Lu_7")
## identify associations ====
interval_bmi <- subset(bmi, pval <= 0.05/102)
interval_whr <- subset(whr, pval <= 0.05/102)
interval_bf <- subset(bf, pval <= 0.05/102)
## overlapping associations ====
...
In the INTERVAL analysis, tests reaching the multiple testing threshold were observed................
...
(ref:MR-figure-forestplot-kettunen-sensitivity-directional-consistency-cap) **Directionaly consistent effects across all MR models in the Kettunen analysis**. Effect estimates and 95% confidence intervals for metabolites which showed directionaly consistent results across the main (IVW-MRE = inverse variance weighted multiplicative random effects) and sensitivity analysis (MR-Egger, weighted median, weighted mode) within each exposure. Of the directionally consistent results within exposures, a total of 29 were identified within all three exposures and are presented here. Solid points indicate a multiple testing threshold (`r round(0.05/22,4)`) has been reached. BMI = body mass index; WHR = waist hip ratio; BF = body fat percentage.
...
(ref:MR-figure-forestplot-interval-sensitivity-directional-consistency-cap) **Directionaly consistent effects across all MR models in the INTERVAL analysis**. Effect estimates and 95% confidence intervals for metabolites which showed directionaly consistent results across the main (IVW-MRE = inverse variance weighted multiplicative random effects) and sensitivity analysis (MR-Egger, weighted median, weighted mode) within each exposure. Of the directionally consistent results within exposures, a total of 29 were identified within all three exposures and are presented here. Solid points indicate a multiple testing threshold (XXXX) has been reached. BMI = body mass index; WHR = waist hip ratio; BF = body fat percentage.
...
In total, 110 metabolites were shared across the Kettunen and INTERVAL analyses. Meta-analysis of *p-values* was performed using Fishers method (Equation \@ref(eq:fishers-method)). A total of `r nrow(pos)` tests, across all exposures had a positive direction of effect in the Kettunen and INTERVAL analyses, `r nrow(neg)` tests had a negative direction of effect. For BMI, `r nrow(pos_bmi)` tests had positive directions of effect and `r nrow(neg_bmi)` tests had negative directions of effect. Similar reuslts were found for WHR (positive = `r nrow(pos_whr)`; negative = `r nrow(neg_whr)`. For BF, a much larger number of tests had negative directions of effect (N = `r nrow(neg_bf)`) than positive (`r nrow(neg_bf)`). A total of `r nrow(data_sig)` tests reached a Bonferroni ($0.05/110$) multiple testing threshold. Of these, `r nrow(sig_pos)` tests had positive directions of effect across Kettunen and INTERVAL analyses and `r nrow(sig_neg)` tests had negative directions of effect. Of these tests, a total of `r length(sig_pos_bmi_whr)` metabolites had a positive direction of effect across Kettunen and INTERVAL analyses while `r length(sig_neg_bmi_whr)` metabolites had negative directions of effect (Table \@ref(tab:MR-table-meta-analysis-sig-direction)). No tests using BF as an exposure reached the multiple testing threshold. \par
...
knitr::kable(table, longtable = T, booktabs = T, format = "latex", escape = F,
    caption = 'Metabolites with a consistent direction of effect and which reached a multiple testing threshold in meta-analysis', row.names = F,
...
All `r length(sig_pos_bmi_whr) + length(sig_neg_bmi_whr)` metabolites which were directionally consistent and reached a multiple testing threshold in the meta-analysis were analysed in the observational analysis in Chapter \@ref(observational). In the observational analysis, a multiple testing threshold of $0.05/53$ (`r round(0.05/53,2)`) was used. For the `r nrow(bmi)` metabolites identified in the MR analyses for BMI, only one (*Total lipids in medium HDL*) did not reach the multiple testing threshold in the observational analysis. All `nrow(whr)` metabolites identified in the MR analyses for WHR also met the multiple testing threshold in the observational analyses. All BMI and WHR associated metabolites in the MR analysis where directionally consistent with effect estimates in th observational analysis (Figure \@ref(fig:MR-figure-associated-metabolites-comparison)). \par

No metabolite associations reached the multiple testing threshold for BF in the MR meta-analysis. *Aceoacetate* was found to be directionally consistent with both MR analyses however this was not measured in the observational analysis and was excluded. As such, a total of `r nrow(bf)` metabolites directionally consistent in the BF MR analyses where available in the observational analysis. Of these, `r table(bf$meta_direction)[1]` had consistent negative directions across MR and observational analyses and `r table(bf$meta_direction)[4]` had consistent positive directions. Of these, `r table(bf_obs_sig$meta_direction)[1]` negative and `r table(bf_obs_sig$meta_direction)[4]` positive consistent metabolites reached the multiple testing threshold in the observational analysis. \par

(ref:MR-figure-associated-metabolites-comparison-cap) **Metabolites associated with measures of adiposity in observational, MR, and MR meta-analysis**. Figure shows all metabolites with consistent directions of effect (BMI, WHR, and BF) and which reached a multiple testing threshold (BMI and WHR) in the MR meta-analysis and their directions and multiple testing thresholds in the observational analysis. \textcolor{yellow}{Yellow} tiles indicate a positive direction of effect; \textcolor{blue}{Blue} tiles indicate a negative direction of effect. * = multiple testing threshold reached: observational = `r round(0.05/53,4)`; Kettunen = `r round(0.05/22,4)`; INTERVAL = `r round(0.05/230,4)`; meta-analysis = `r round(0.05/110,4)`
...
knitr::include_graphics("data/MR/figures/associated_metabolites_comparison1.pdf")
```
...
### Summary
There is clear evidence of a association between increased adiptosity and the global metabolic profile. This evidence is strengthened for the handful of metabolites that were comparable across MR, observational, and the work conducted by Wurtz et al. (2014), suggesting robust associations with adiposity. Some of these metabolites may be involved in the development of adiposity associated diseases, however their roles may also be as markers of adiposity or other co-occuring incidents that may be the ultimate intermediates on the pathway to disease development. It is important that when examining assocaitions between metabolites and disease the interrelated nature of metabolites, and the likelihood that they are merely markers of physiological change is taken into account both when performing analyses and examaning results. <br>
