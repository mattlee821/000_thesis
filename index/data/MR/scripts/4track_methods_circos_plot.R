circos_plot <- function(track_number,
  ...
  ## Colours
  # discrete_palette <- c("#F2AD00", # track 1 colour
  #                       "#FF0000", # track 2 colour
  #                       "#5BBCD6", # track 3 colour
  #                       "#00be50") # track 4 colour
  ...
  if (equal_axis == TRUE && track_number == 2){
  ...
  else if (equal_axis == TRUE && track_number == 4) {
    # identify track 1 axis limits
    a <- min(track1_data[[lower_ci]])
    b <- min(track1_data[[upper_ci]])
    axis_min <- min(a, b)
    ...
    # identify track 4 axis limits
    a <- min(track4_data[[lower_ci]])
    b <- min(track4_data[[upper_ci]])
    axis_min <- min(a, b)
    axis_min <- round(axis_min, 3)
    track4_axis_min <- round(axis_min + (axis_min * 0.1), 3)
    
    a <- max(track4_data[[lower_ci]])
    b <- max(track4_data[[upper_ci]])
    axis_max <- max(a, b)
    axis_max <- round(axis_max, 3)
    track4_axis_max <- round(axis_max + (axis_max * 0.01), 3)
    # identify shared axis limits
    axis_min <- min(track1_axis_min, track2_axis_min, track3_axis_min, track4_axis_min)
    axis_max <- max(track1_axis_max, track2_axis_max, track3_axis_max, track4_axis_max)
    
    # assign axis limits for each track
    track1_axis_min <- axis_min
    track2_axis_min <- axis_min
    track3_axis_min <- axis_min
    track4_axis_min <- axis_min
    track1_axis_max <- axis_max
    track2_axis_max <- axis_max
    track3_axis_max <- axis_max
    track4_axis_max <- axis_max
  }
  else if (equal_axis == FALSE && track_number == 4){
    # identify track 1 axis limits
    a <- min(track1_data[[lower_ci]])
    b <- min(track1_data[[upper_ci]])
    axis_min <- min(a, b)
  ...
  # track 4 ====
  ## points ====
  if(track_number >= 4 && track4_type == "points"){
    data <- track4_data
    track.index <- 5
    ...
    for(i in 1:nlevels(data$section_numbers)){
      ...
      circlize::circos.trackPlotRegion(factors = data1$section_numbers, #we plot the first region based on the column in our data set which we creat the sections from
                                       ...
                                       track.height = track4_height, #how big is the track as % of circle
  ...
  ## lines ====
  if(track_number >= 4 && track4_type == "lines"){
    data <- track4_data
    track.index <- 5
  ...
  ## barplot ====
  if(track_number >= 4 && track4_type == "bar"){
    data <- track1_data
    track.index <- 5
  ...
  ## histogram ====
  if(track_number >= 4 && track4_type == "histogram"){
    data <- track4_data
    track.index <- 5
  ...
  ## 6.43 - Assign the legend points
  if(legend == TRUE && track_number >= 4){
    
    legend1 <- ComplexHeatmap::Legend(at = c(track1_label, track2_label, track3_label, track4_label),
                                      labels_gp = grid::gpar(fontsize = 15),
                                      ncol = 1,
                                      border = NA, # color of legend borders, also for the ticks in the continuous legend
                                      background = NA, # background colors
                                      legend_gp = grid::gpar(col = c(discrete_palette[1], discrete_palette[2], discrete_palette[3], discrete_palette[4])), # graphic parameters for the legend
                                      type = "points", # type of legends, can be grid, points and lines
                                      pch = 19, # type of points
                                      size = grid::unit(15, "mm"), # size of points
                                      grid_height	= grid::unit(15, "mm"),
