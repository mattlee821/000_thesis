...
The Appendix is split into sections (A.1, A.2...) corresponding to each chapter. All tables and figures presented here are also available on [GitHub](https://github.com/mattlee821/000_thesis/tree/master/index/data) within each chapters respective folder. Where tables are too big to be presented in the Appendix (i.e. SNP lists) they are referenced and a GitHub link is provided. \par
...
data <- read.csv("../../005_systematic_review/analysis/meta_analysis/results/quality_assessment_results.csv", header = T)
data <- data[,c(3:18)]
...
## Chapter 3: EpiViz {#appendix-EpiViz}
...
knitr::kable(data, longtable = T, booktabs = T, format = "latex", escape = T,
    caption = 'Metabolites available after quality control across all age groups', 
    row.names = F,
    col.names = c("Metabolite","Label", "Class","Subclass", "Derived")) %>%
  kable_styling(full_width = T, latex_options = c("striped", "HOLD_poistion", "repeat_header"), position = "center") %>%
    kableExtra::footnote(general = "Derived indictaes whether a metabolite is directly measured (no) or is a derived measure (yes).", threeparttable = T, general_title = "", footnote_as_chunk = T) %>%
...
## Chapter 6: Instrumentation {#appendix-instrumentation}
## Chapter 7: Mediation {#appendix-mediation}

### Methods
```{r appendix-mediation-table-metabolites, echo=FALSE}
data <- read.table("data/mediation/tables/metabolites.txt", header = T, sep = "\t")
data <- data[order(data$Class, data$Subclass, data$Metabolite),]
knitr::kable(data, longtable = T, booktabs = T, format = "html", escape = T,
