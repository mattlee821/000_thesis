# script to obtain data from ALSPAC individuals - measures of adiposity: BMI, WHR, BF
...
#### z-score
mums_bmi$bmi_z <- (mums_bmi$bmi - mean(mums_bmi$bmi, na.rm = T))/sd(mums_bmi$bmi, na.rm = T)
...
#### z-score
mums_whr$whr_z <- (mums_whr$whr - mean(mums_whr$whr, na.rm = T))/sd(mums_whr$whr, na.rm = T)
...
### make BF variable
mums_bf$bf <- mums_bf$dx020 / (mums_bf$dx020 + mums_bf$dx331) * 100
mums_bf <- mums_bf[,c("aln", "bf")]
n_missing_bf <- sum(is.na(mums_bf$bf))
#### z-score
mums_bf$bf_z <- (mums_bf$bf - mean(mums_bf$bf, na.rm = T))/sd(mums_bf$bf, na.rm = T)

## combined adiposity data frame
mums <- as.data.frame(mums[,1])
colnames(mums) <- "aln"
mums <- left_join(mums, mums_bmi, by = "aln")
...
adult_female_N <- data.frame(Group = c("Mothers", "Mothers"),
                             ...
                             N_bf = c(NA,NA),
                             N_bf = c(nrow(mums) - n_missing_bf, nrow(mums) - n_missing_bf),
...
rm(a,b,c,d,e,fom1,fom2,mums_bf,mums_bmi,mums_hc,
   ...
   n_missing_bf,n_missing_bmi,n_missing_whr)
...
#### z-score
dads_bmi$bmi_z <- (dads_bmi$bmi - mean(dads_bmi$bmi, na.rm = T))/sd(dads_bmi$bmi, na.rm = T)
...
#### z-score
dads_whr$whr_z <- (dads_whr$whr - mean(dads_whr$whr, na.rm = T))/sd(dads_whr$whr, na.rm = T)
...
### make BF variable
dads_bf$bf <- dads_bf$ff1dx020 / (dads_bf$ff1dx020 + dads_bf$ff1dx331) * 100
dads_bf <- dads_bf[,c("aln", "bf")]
n_missing_bf <- sum(is.na(dads_bf$bf))
#### z-score
dads_bf$bf_z <- (dads_bf$bf - mean(dads_bf$bf, na.rm = T))/sd(dads_bf$bf, na.rm = T)

## combined adiposity data frame
dads <- as.data.frame(dads[,1])
colnames(dads) <- "aln"
dads <- left_join(dads, dads_bmi, by = "aln")
...
rm(dads_bf,dads_bmi,dads_total_fat_free_mass,dads_total_fat_mass,
   ...
   n_missing_bmi,n_missing_whr,n_missing_bf)
...
adults_N <- data.frame(Group = c("Adults", "Adults"),
                             ...
                             N_bf = c(NA,NA),
                             N_bf = c(nrow(adults) - sum(is.na(adults$bf)),nrow(adults) - sum(is.na(adults$bf))),
...
f7_bf$bf <- -156.1 - 89.1 * log(f7_bf$height) + 45.6 * log(f7_bf$weight) + 0.120 * f7_bf$age + 0.0494 * f7_bf$impedance + (19.6 * log(f7_bf$height))
#### recode missing as NA
f7_bf$bf[f7_bf$bf <= 0] <- NA
### make WHR variable
children_bf <- f7_bf[,c("aln_qlet", "bf", "impedance")]
n_missing_bf <- sum(is.na(children_bf$bf))
n_missing_impedance <- sum(is.na(children_bf$impedance))
#### z-score
children_bf$bf_z <- (children_bf$bf - mean(children_bf$bf, na.rm = T))/sd(children_bf$bf, na.rm = T)
children_bf$impedance_z <- (children_bf$impedance - mean(children_bf$impedance, na.rm = T))/sd(children_bf$impedance, na.rm = T)
...
children_N <- data.frame(Group = c("Children", "Children"),
                             ...
                             N_bf = c(NA,NA),
                             N_bf = c(nrow(children) - n_missing_bf, nrow(children) - n_missing_bf),
...
rm(a,b,c,d,e,f7,f8,f7_ID,f8_ID,
   bmi_f7,bmi_f8,f7_wc,f7_hc,f7_whr,f7_impedance,
   children,children_bmi,children_bf,children_whr,
...
adolescents_N <- data.frame(Group = c("Adolescents", "Adolescents"),
                         ...
                         N_bf = c(NA,NA),
                         N_bf = c(nrow(adolescents) - n_missing_bf, nrow(adolescents) - n_missing_bf),
...
rm(a,b,c,d,e,tf3,tf4,tf4_ID,tf3_ID,
   adolescents,adolescents_bf,adolescents_bf,adolescents_bmi,
   bf_tf3,bf_tf4,bf_tf3,bf_tf4,bmi_tf3,bmi_tf4,
   n_missing_bf,n_missing_bf,n_missing_bmi,n_missing_whr,
...
#### z-score
bmi_f24$bmi_z <- (bmi_f24$bmi - mean(bmi_f24$bmi, na.rm = T))/sd(bmi_f24$bmi, na.rm = T)
...
#### z-score
young_adult_whr$whr_z <- (young_adult_whr$whr - mean(young_adult_whr$whr, na.rm = T))/sd(young_adult_whr$whr, na.rm = T)

### body fat DXA
bf_f24 <- f24[,c("aln_qlet", "FKDX1002", "FKDX1001")]
colnames(bf_f24) <- c("aln_qlet", "lean_mass_grams", "fat_mass_grams")
#### recode missing as NA
bf_f24$lean_mass_grams[bf_f24$lean_mass_grams <= 0] <- NA
bf_f24$fat_mass_grams[bf_f24$fat_mass_grams <= 0] <- NA
bf_f24$bf <- bf_f24$fat_mass_grams / (bf_f24$lean_mass_grams + bf_f24$fat_mass_grams) * 100
n_missing_bf <- sum(is.na(bf_f24$bf))
young_adult_bf <- bf_f24[,c("aln_qlet", "bf")]
#### z-score
young_adult_bf$bf_z <- (young_adult_bf$bf - mean(young_adult_bf$bf, na.rm = T))/sd(young_adult_bf$bf, na.rm = T)

## combined adiposity data frame
young_adults <- as.data.frame(child_data[,1])
colnames(young_adults) <- "aln_qlet"
young_adults <- left_join(young_adults, bmi_f24, by = "aln_qlet")
young_adults <- left_join(young_adults, young_adult_whr, by = "aln_qlet")
young_adults <- left_join(young_adults, young_adult_bf, by = "aln_qlet")
...
young_adults_N <- data.frame(Group = "Young adults",
                           ...
                           N_bf = NA,
                           N_bf = nrow(young_adults) - n_missing_bf,
                           subgroup = "Focus on Fathers 1",
                           N_subgroup = n_f24,
...
rm(n_missing_bf,n_missing_bmi,n_missing_whr,n_f24,n_woc_f24,
   young_adult_bf, young_adult_whr,bmi_f24,bf_f24,f24,
