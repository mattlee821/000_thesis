...
## bmi
bmi_kettunen <- mr_bmi[,c("exposure", "outcome", "label", "raw.label", "kettunen_b", "kettunen_p")]
bmi_kettunen$label <- paste(bmi_kettunen$label, "Kettunen")
colnames(bmi_kettunen) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
bmi_kettunen$p_label[bmi_kettunen$p < 0.05/22] <- "*"
bmi_kettunen$p_label[bmi_kettunen$p > 0.05/22] <- ""

bmi_interval <- mr_bmi[,c("exposure", "outcome", "label", "raw.label", "interval_b", "interval_p")]
bmi_interval$label <- paste(bmi_interval$label, "INTERVAL")
colnames(bmi_interval) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
bmi_interval$p_label[bmi_interval$p < 0.05/28] <- "*"
bmi_interval$p_label[bmi_interval$p > 0.05/28] <- ""

bmi_meta <- mr_bmi[,c("exposure", "outcome", "label", "raw.label", "meta_direction", "meta_p")]
bmi_meta$label <- paste(bmi_meta$label, "Meta-analysis")
colnames(bmi_meta)[5] <- "b"
colnames(bmi_meta)[6] <- "p"
...
bmi_obs <- obs_bmi[,c("exposure", "outcome", "label", "raw.label", "observational_b", "observational_p")]
colnames(bmi_obs) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
...
## whr
whr_kettunen <- mr_whr[,c("exposure", "outcome", "label", "raw.label", "kettunen_b", "kettunen_p")]
whr_kettunen$label <- paste(whr_kettunen$label, "Kettunen")
colnames(whr_kettunen) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
whr_kettunen$p_label[whr_kettunen$p < 0.05/22] <- "*"
whr_kettunen$p_label[whr_kettunen$p > 0.05/22] <- ""

whr_interval <- mr_whr[,c("exposure", "outcome", "label", "raw.label", "interval_b", "interval_p")]
whr_interval$label <- paste(whr_interval$label, "INTERVAL")
colnames(whr_interval) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
whr_interval$p_label[whr_interval$p < 0.05/28] <- "*"
whr_interval$p_label[whr_interval$p > 0.05/28] <- ""

whr_meta <- mr_whr[,c("exposure", "outcome", "label", "raw.label", "meta_direction", "meta_p")]
whr_meta$label <- paste(whr_meta$label, "Meta-analysis")
colnames(whr_meta)[5] <- "b"
colnames(whr_meta)[6] <- "p"
...
whr_obs <- obs_whr[,c("exposure", "outcome", "label", "raw.label", "observational_b", "observational_p")]
colnames(whr_obs) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
...
## bf
bf_kettunen <- mr_bf[,c("exposure", "outcome", "label", "raw.label", "kettunen_b", "kettunen_p")]
bf_kettunen$label <- paste(bf_kettunen$label, "Kettunen")
colnames(bf_kettunen) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
bf_kettunen$p_label[bf_kettunen$p < 0.05/22] <- "*"
bf_kettunen$p_label[bf_kettunen$p > 0.05/22] <- ""

bf_interval <- mr_bf[,c("exposure", "outcome", "label", "raw.label", "interval_b", "interval_p")]
bf_interval$label <- paste(bf_interval$label, "INTERVAL")
colnames(bf_interval) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
bf_interval$p_label[bf_interval$p < 0.05/28] <- "*"
bf_interval$p_label[bf_interval$p > 0.05/28] <- ""

bf_meta <- mr_bf[,c("exposure", "outcome", "label", "raw.label", "meta_direction", "meta_p")]
bf_meta$label <- paste(bf_meta$label, "Meta-analysis")
colnames(bf_meta)[5] <- "b"
colnames(bf_meta)[6] <- "p"
...
bf_obs <- obs_bf[,c("exposure", "outcome", "label", "raw.label", "observational_b", "observational_p")]
colnames(bf_obs) <- c("exposure", "outcome", "label", "raw.label", "b", "p")
...
a <- c("Observational BMI", "MR BMI Meta-analysis",
           "Observational WHR", "MR WHR Meta-analysis",
           "Observational BF", "MR BF Meta-analysis")
plot_data1 <- plot_data[plot_data$label %in% a, ]

p1 <- ggplot(plot_data1, aes(label, raw.label)) +
  ...
  ggforce::facet_col(
    ...
    scales = "free_y",
    space = "free")
ggsave("index/data/MR/figures/associated_metabolites_comparison.pdf",p1, width=8, height=14, units="in", scale=1)
