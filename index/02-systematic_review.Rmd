...
a <- year[,c("year", "outcome_n_case_current")]
a <- a[complete.cases(a), ]
outcome_case <- a %>% 
  group_by(year) %>% 
  summarise(outcome_case_average = mean(outcome_n_case_current))

a <- year[,c("year", "outcome_n_control_current")]
a <- a[complete.cases(a), ]
outcome_control <- a %>% 
  group_by(year) %>% 
  summarise(outcome_control_average = mean(outcome_n_control_current))
...
plot_data <- left_join(plot_data, outcome_case, by = "year")
plot_data <- left_join(plot_data, outcome_control, by = "year")
...
ggplot(plot_data) +
  ...
  geom_point(aes(x = year, y = outcome_control_average/5000, group = 1), size = 2, colour = discrete_wes_pal[2]) +
  geom_point(aes(x = year, y = outcome_case_average/5000, group = 1), size = 2, colour = discrete_wes_pal[3]) +
  geom_line(aes(x = year, y = exposure_average/5000, group = 1), size = 3, colour = discrete_wes_pal[1]) +
  geom_line(aes(x = year, y = outcome_control_average/5000, group = 1), size = 3, colour = discrete_wes_pal[2]) +
  geom_line(aes(x = year, y = outcome_case_average/5000, group = 1), size = 3, colour = discrete_wes_pal[3]) +
...
### Quality assessment
```{r echo=FALSE}
rob_data <- read.table("../../005_systematic_review/analysis/rob_results.txt", header = T, sep = "\t")
```
