...
it might feel boring but make sure each of these paragraphs has the same format so that the reader knows what to expect - there are still some inconsistencies so it feels like information is either missing or you're just changing the way you're reporting things between each paragraph. 

e.g., total number of analyses across studies,  what this included, how many were positive and negative (this is something that is omitted in some of the latter paragraphs), some insightful examples of these, what this might mean, an "on balance" summary statement (this is also something that seems to be omitted from some of the paragraphs). 
